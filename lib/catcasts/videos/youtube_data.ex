defmodule Catcasts.Videos.YoutubeData do
  ...
  defp get_json_data(video_id) do
    HTTPoison.get! "https://www.googleapis.com/youtube/v3/videos?id=#{video_id}&key=#{Application.fetch_env!(:catcasts, :youtube_api_key)}&part=snippet,statistics,contentDetails&fields=items(id,snippet(title,thumbnails(high)),statistics(viewCount),contentDetails(duration))"
  end
